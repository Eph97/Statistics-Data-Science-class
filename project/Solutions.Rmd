...
We are obviously not the first to ask these types of questions so we can use ideas that other sports statisticians have developed.
...
# Playoff star dates found on Wikepedia
...
nba_data_reshape <- nba_data_reshape %>% 
  left_join(playoff_dates, by = "SEASON")

# identify regular season games, playoff games and preseason games
nba_data_reshape <- nba_data_reshape %>% 
  mutate(GAME_DATE_EST = lubridate::ymd(GAME_DATE_EST)) %>% 
  mutate(Playoff = GAME_DATE_EST >= playoff_start_date,
         Preseason = GAME_DATE_EST < season_start_date) %>% 
  mutate(RegularSeason = Playoff == FALSE & Preseason == FALSE)

# join team names
nba_data_reshape <- nba_data_reshape %>% 
  left_join(nba_teams, by = "TEAM_ID")
```
...
expected_wins_nba <- nba_data_reshape %>% 
  ...
  summarise(GamesPlayed = n(),
            TotalWins = sum(Win),
            TotalPoints = sum(POINTS_FOR),
            TotalPointsAgainst = sum(POINTS_AGAINST)) %>% 
...
```{r}
# fit a linear model to assess explainability
...
## Come Back To How close to reality in the NBA?

Another way we can assess how well our predicitions track with reality is to look at the Root Mean Squared Error (RMSE). This is 

The RMSE for the NBA is `r Metrics::rmse(expected_wins_nba$TotalWins, expected_wins_nba$ExpectedWins)`, slightly lower than for the college game, and even more impressive when you take in to account that the NBA season is 82 games long, while NCAA seasons are usually played over less than 40 games.
...
```{r, warning=FALSE, message=FALSE}
expected_wins_nba <- expected_wins_nba %>% mutate(WinsAboveExpected = TotalWins - ExpectedWins)
...
predicted_wins_nba <- expected_wins_nba %>%
  mutate(ThisSeasonPerformance = ifelse(WinsAboveExpected < -3, "Under Performed", 
                                        ifelse(WinsAboveExpected > 3, "Over Performed", "Close to Expected"))) %>% 
...
That nice example aside, the RMSE of the predicted wins is `r round(Metrics::rmse(predicted_wins_nba$TotalWins, predicted_wins_nba$PredictedWins),2)` games, with an $Adjusted\ R^{2}$ of `r round(summary(lm(TotalWins ~ PreviousEWF, data = predicted_wins_nba))$adj.r.squared,4)`.

To truly see how well we can predict the subsequent year's performance, we can look at the distribution.
...
To do this, we must determine what "over" or "under" performing is. The below plots try to show a number of different cutoff levels for game differences - one through five - meaning if the team's actual wins were greater than one game above their expected wins, they *over performed* that season, while if the team were greater than one full loss below expected, they *under performed* that season. The same logic holds for all the other cutoffs displayed below.

So with this, we can see that as a team moves further away from their expected number of wins in the previous season, 
they are then more likely to perform *close to expectation* the following season.

When the team won or lost one or more games beyond expected (top plot), they were equally likely to either under or over perform the following season.
Once the deviation from expected grew beyond two games, the likelihood of the team performing closer to expected the following season grew from \~48%, to \~90%
